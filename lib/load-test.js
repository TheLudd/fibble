...
export const prop1 = global.__tribbleState.getReplacementValue('someUrl', 'prop1')
export const prop2 = global.__tribbleState.getReplacementValue('someUrl', 'prop2')
    `.trim()
    const expected = {
  ...
  it('handles default replacements', async () => {
    ...
    const result = await load('someUrl', 'context', defaultLoad)
    const expectedSource = `
export default global.__tribbleState.getReplacementValue('someUrl', 'default')
    `.trim()
    const expected = {
      source: expectedSource,
      format: 'module',
...
export const prop1 = global.__tribbleState.getReplacementValue('someUrl', 'prop1')
  ...
  it('sets parent url in the replace file when it is loaded', async () => {
    ...
    const { href: url, pathname } = new URL(
      `./replace.js?__tribbleReplaceCaller=${parentURL}`,
      import.meta.url,
    )
    const context = {
